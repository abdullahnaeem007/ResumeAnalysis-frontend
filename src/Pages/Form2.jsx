    ...
    <div class='w-full min-h-screen flex flex-col space-y-[6vh] font-Poppins'>
        ...
        <div class='w-full flex flex-col items-center space-y-[10vh] z-10'>
            <div class='flex flex-col items-center'>
                <text class='text-[#167FFC] text-[2.5vw] font-semibold'>We're Getting Closer!</text>
            ...
            <div class='w-1/3 flex flex-col space-y-[8vh]'>
                <div class='flex flex-col space-y-[3vh]'>
                    <div class='flex flex-col space-y-[1vh]'>
                        ...
                        <div class='flex items-center space-x-[1vw]'>
                            <input value={User?.user.email} onChange={(e)=>{setEmail(e.target.value)}} class='w-full px-[1vw] py-[1.5vh] bg-[#EAF0FF] rounded-lg outline-none'/>
                            {
                                Email.length==0?
                    ...
                    <div class='flex flex-col space-y-[1vh]'>
                        <text class='text-[#928D8D]'>Physical Address (Optional):</text>
                        <div class='flex items-center space-x-[1vw]'>
                            <input onChange={(e)=>{setAddress(e.target.value)}} class='w-full px-[1vw] py-[1.5vh] bg-[#EAF0FF] rounded-lg outline-none'/>
                            <AiOutlineCheckCircle size='1.9rem' color='green'/>
                        </div>
                    </div>
                </div>
                <div class='w-full flex justify-between'>
                    ...
                    <button onClick={()=>{if(Email.length>0){localStorage.setItem('email',JSON.stringify(Email));window.location.replace('/Upload')}}} class='px-[2vw] py-[1.5vh] rounded-lg bg-[#167FFC] text-white'>Continue</button>
                </div>
            </div>
        </div>
